...
- words
    ...
    2. .wordpress.com\</em> doesn&#8217;t&nbsp;exist
- condition - and
...
**Type - word**
- words
    1. acf-to-rest-api\/class-acf-to-rest-api.php
- part - body
- condition - and
...
- regex
    1. root:.*:0:0:
    2. define\('DB_(NAME|USER|PASSWORD|HOST|CHARSET|COLLATE)'
- condition - or
- part - body
...
- words
    1. Index of
    2. wp-content/uploads/wp-file-manager-pro/fm_backup
    3. backup_
- condition - and
...
**Type - word**
- words
    1. \<div id="adminmenumain" role="navigation" aria-label="Main menu">
    2. \<h1>Dashboard\</h1>
- part - body
- condition - and
...
- part - body
- regex
    1. rest_post_invalid_id
    2. "(guid|title|content|excerpt)":{"rendered":
- condition - or
...
- {{BaseURL}}/wp-admin/options-general.php?page=smartcode
  - sgcgoogleanalytic=<script>console.log("Nuclei - Open-source project [github.com/projectdiscovery/nuclei]")</script>&sgcwebtools=&button=Save+Changes&action=savegooglecode
...
---
# WordPress Plugin WP Security Audit Log 3.1.1 - Information Disclosure
## Description
- Access to wp-content/uploads/wp-security-audit-log/* files is not restricted. For example, these files are indexed by Google and allows for attackers to possibly find sensitive information
- severity - medium
...
## Description
- WordPress Core \< 4.7.1 is susceptible to user enumeration because it does not properly restrict listings of post authors via wp-includes/rest-api/endpoints/class-wp-rest-users-controller.php in the REST API, which allows remote attackers to obtain sensitive information via a wp-json/wp/v2/users request.
- severity - medium
...
---
# Redux Framework - Unauthenticated Sensitive Information Disclosure
## Description
...
- part - body
- words
    1. Welcome to your WordPress Dashboard
...
---
# WordPress Email Subscribers & Newsletters \<4.2.2 - Arbitrary File Retrieval
## Description
- The WordPress plugin Email Subscribers & Newsletters before 4.2.3 contains a flaw that allows unauthenticated file download and user information disclosure.
- severity - medium
...
**Type - word**
- words
    1. Content-Disposition: attachment; filename=all-contacts.csv;
- part - header
...
- part - body
- words
    1. directorist-authors__card__details__top
    2. directorist-authors__card__info-list
- condition - and
...
- regex
    1. response_code":200
    2. "email":"([a-zA-Z-_0-9@.]+)","display_name":"([a-zA-Z-_0-9@.]+)","gravatar_url":"http?:\\\/\\\/([a-z0-9A-Z.\\\/?=&@_-]+)"
- condition - and
...
**Type - regex**
- regex
    1. \<generator>https?:\/\/wordpress\.org.*\</generator>
    2. wp-login.php
    3. \/wp-content/themes\/
    4. \/wp-includes\/
    5. name="generator" content="wordpress
    6. \<link[^>]+s\d+\.wp\.com
    7. \<!-- This site is optimized with the Yoast (?:WordPress )?SEO plugin v([\d.]+) -
    8. \<!--[^>]+WP-Super-Cache
- condition - or
...
- group - 1
- regex
    1. (?m)https:\/\/wordpress.org\/\?v=([0-9.]+)

**Type - regex**
- name - version_by_js
- group - 1
- regex
    1. wp-emoji-release\.min\.js\?ver=((\d+\.?)+)\b

**Type - regex**
- name - version_by_css
- group - 1
- regex
    1. install\.min\.css\?ver=((\d+\.?)+)\b
